...
const game = {
  ...
  updatePreviewPieces() {
    let counter = this.piece.nextThree.length;
    while (counter++ < 3) {
      this.piece.nextThree.push(this.selectRandomPiece());
    }
  ...
  updateDisplayPreviewPieces() {
    ...
    imgs.forEach((img, idx) => {
      img.src = path + this.piece.nextThree[idx].type + ext;
    });
  },
  ...
  newPiece() {
    ...
    this.piece.current = this.piece.nextThree.shift();
  ...
  drawAll() {
    this.drawCanvas();
    playfield.draw();
    this.piece.current.draw("original");
    this.piece.current.getGhost();
  },
  ...
  mainLoop(time = 0) {
    ...
    if (this.loop.dropCounter > this.loop.dropInterval) {
      if (this.loop.gravity) this.piece.current.drop();
    }
